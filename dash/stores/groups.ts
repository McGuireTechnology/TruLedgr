...
export const useGroupsStore = defineStore('groups', () => {
  ...
  const publicGroups = computed(() => 
    groups.value?.filter(group => group.is_public) || []
  )

  const userGroups = computed(() => 
    groups.value?.filter(group => group.owner_id === getCurrentUserId()) || []
  )

  const memberGroups = computed(() => 
    groups.value?.filter(group => 
      group.members?.some(member => member.user_id === getCurrentUserId())
    ) || []
  )
  ...
  async function fetchGroup(groupId: string, includeMembers = false) {
    try {
      loading.value = true
      error.value = null

      const params = includeMembers ? '?include_members=true' : ''
      const response = await apiClient.get<GroupWithMembers>(`/groups/${groupId}${params}`)
  ...
  async function fetchGroupBySlug(slug: string, includeMembers = false) {
    try {
      loading.value = true
      error.value = null

      const params = includeMembers ? '?include_members=true' : ''
      const response = await apiClient.get<GroupWithMembers>(`/groups/slug/${slug}${params}`)
  ...
  async function createGroup(groupData: GroupCreate): Promise<Group | null> {
    try {
      loading.value = true
      error.value = null

      const response = await apiClient.post<Group>('/groups', groupData)
  ...
  async function updateGroup(groupId: string, groupData: GroupUpdate): Promise<Group | null> {
    try {
      loading.value = true
      error.value = null

      const response = await apiClient.put<Group>(`/groups/${groupId}`, groupData)
  ...
  async function addUsersToGroup(
    groupId: string, 
    membershipRequest: GroupMembershipRequest
  ): Promise<UserInGroup[] | null> {
    try {
      loading.value = true
      error.value = null

      const response = await apiClient.post<UserInGroup[]>(
  ...
  async function updateUserMembership(
    groupId: string, 
    userId: string, 
    membershipData: GroupMembershipUpdate
  ): Promise<UserInGroup | null> {
    try {
      loading.value = true
      error.value = null

      const response = await apiClient.put<UserInGroup>(
  ...
  function setGroupType(groupType: string | null) {
    filters.value.groupType = groupType
  }

  function setIsPublic(isPublic: boolean | null) {
    filters.value.isPublic = isPublic
  }

  function setOwnerId(ownerId: string | null) {
    filters.value.ownerId = ownerId
  }

  function setOrderBy(orderBy: string, orderDirection: 'asc' | 'desc' = 'desc') {
