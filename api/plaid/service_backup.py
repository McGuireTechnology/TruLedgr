...
class PlaidService:
    ...
    async def delete_item_db(self, user_id: int, item_id: int) -> bool:
    ...
    async def sync_item_accounts_db(self, user_id: int, item_id: int):
        """Sync accounts for a specific item"""
        self._ensure_modular_services()
        return await self.accounts_service.sync_user_item_accounts(user_id, str(item_id))

    async def sync_item_transactions_db(self, user_id: int, item_id: int, days: int = 30) -> int:
        """Sync transactions for a specific item"""
        self._ensure_modular_services()
        return await self.transactions_service.sync_item_transactions(user_id, str(item_id), days)
